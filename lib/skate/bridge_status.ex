defmodule Skate.BridgeStatus do
  ...
  ## Options
  - `bridge_route_ids` defaults to `[]`
  - `blackout_period` defaults to `-120` seconds
  ...
  def maybe_record_bridge_status(attrs, opts \\ []) do
    ...
    |> Ecto.Multi.append(Notifications.Notification.new_bridge_movement_notification(attrs))
  ...
  defp cutoff_time(opts) do
    # This "blackout" period is so that multiple Skate instances
    # don't duplicate their work when polling the bridge API
    blackout_period = Keyword.get(opts, :blackout_period, Duration.new!(second: -120))
  ...
  defp add_users_to_notification_params(attrs, opts) do
    bridge_route_ids = Keyword.get(opts, :bridge_route_ids, [])
