defmodule Static.Data do
  ...
  def parse_files(%{gtfs: gtfs_files, hastus: hastus_files}) do
    _hastus_activities = Static.Hastus.Activity.parse(hastus_files["activities.csv"])
    hastus_trips = Static.Hastus.Trip.parse(hastus_files["trips.csv"])
  ...
  defp bus_trips(trips_data, stop_times_data, bus_route_ids, run_ids) do
    gtfs_bus_trips =
      Csv.parse(
        trips_data,
        filter: &Static.Gtfs.Trip.row_in_route_id_set?(&1, bus_route_ids),
        parse: &Static.Gtfs.Trip.from_csv_row/1
      )
