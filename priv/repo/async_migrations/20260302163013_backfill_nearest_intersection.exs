defmodule Skate.Repo.Migrations.BackfillNearestIntersection.MigratingSchema do
  @moduledoc """
...
defmodule Skate.Repo.Migrations.BackfillNearestIntersection do
  ...
  defp page_query(last_id) do
    from(
      r in Skate.Repo.Migrations.BackfillNearestIntersection.MigratingSchema,
      select: r.id,
      where: r.id > ^last_id,
      order_by: [asc: r.id],
      limit: @batch_size
  ...
  defp do_change(batch_of_ids) do
    ...
    |> repo().all(log: :info)
    |> Enum.map(fn %Skate.Repo.Migrations.BackfillNearestIntersection.MigratingSchema{
                     id: id,
                     state: state
                   } = detour ->
      detour
      ...
      |> Ecto.Changeset.put_change(
        ...
        extract_nearest_intersection(state)
      )
      |> repo().update()
      |> case do
  ...
  defp extract_nearest_intersection(%{
         "context" => %{"nearestIntersection" => nearest_intersection}
       }),
       do: nearest_intersection

  defp extract_nearest_intersection(_detour_context), do: nil
