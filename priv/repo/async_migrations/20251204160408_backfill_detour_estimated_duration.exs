defmodule Skate.Repo.Migrations.BackfillDetourEstimatedDuration.MigratingSchema do
  @moduledoc """
...
defmodule Skate.Repo.Migrations.BackfillDetourEstimatedDuration do
  ...
  defp page_query(last_id) do
    from(
      r in Skate.Repo.Migrations.BackfillDetourEstimatedDuration.MigratingSchema,
      select: r.id,
      where: r.id > ^last_id,
      order_by: [asc: r.id],
      limit: @batch_size
  ...
  defp do_change(batch_of_ids) do
    ...
    |> repo().all(log: :info)
    |> Enum.map(fn %Skate.Repo.Migrations.BackfillDetourEstimatedDuration.MigratingSchema{
                     id: id,
                     state: state
                   } = detour ->
      detour
      ...
      |> Ecto.Changeset.put_change(
        :estimated_duration,
        extract_selected_duration(state)
      )
      |> repo().update()
      |> case do
  ...
  defp extract_selected_duration(%{"context" => %{"selectedDuration" => estimated_duration}}),
    do: estimated_duration

  defp extract_selected_duration(_detour_context), do: nil
