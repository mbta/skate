defmodule Concentrate.Parser.GTFSRealtimeEnhancedTest do
  ...
  describe "decode_vehicle/1" do
    ...
    test "decodes a VehiclePosition JSON map with state_of_charge info set to nil" do
      input =
    ...
    test "decodes a VehiclePosition JSON map with state_of_charge values set to nil if vehicle id present in allow list, but no data present" do
      input =
        build(:gtfs_realtime_enhanced_vehicle_position, %{
          "state_of_charge_percentage" => nil,
          "state_of_charge_timestamp" => nil,
      ...
      assert log =~ "SOC missing for vehicle_id"
    end
