defmodule Static.HealthServerTest do
  use ExUnit.Case

  alias Static.HealthServer
  ...
  test "gtfs server tells the health server it's loaded" do
    ...
    gtfs_pid = Static.start_mocked(%{}, health_server_pid)
    # Make a synchronous call to make sure the data is loaded
    Static.all_routes(gtfs_pid)
    assert HealthServer.ready?(health_server_pid)
  end

  test "can start in supervision tree with default name" do
    start_supervised({Static.HealthServer, []})
    # No Static server points to this health server, so it won't be ready
