defmodule GtfsTest do
  ...
  alias Static.Trip
  alias Static.Gtfs.{Route, RoutePattern, Shape, Stop, StopTime, Timepoint}
  alias Static.Gtfs.Shape.Point
  ...
  describe "all_routes" do
    ...
    test "filters out non-bus routes" do
      ...
      refute Enum.any?(Static.all_routes(pid), &(&1.id == "Red"))
    end
  end
  ...
  describe "timepoints_on_route" do
    test "returns all of the timepoints for route patterns on this route" do
      ...
      assert Static.timepoints_on_route("39", pid) == [
    ...
    test "filters out non-bus trips, route patterns, and timepoints" do
      ...
      assert Static.timepoints_on_route("Blue", pid) == []
    end
    ...
    test "merges multiple trips into a coherent order" do
      ...
      assert Static.timepoints_on_route("route", pid) == [
  ...
  describe "stop/2" do
    ...
    test "returns nil if stop doesn't exist" do
      pid = Static.start_mocked(%{})
      assert Static.stop("id", pid) == nil
    end
  end
  ...
  describe "trip" do
    ...
    test "includes run from hastus" do
      ...
      assert Static.trip("trip", pid).run_id == "123-1501"
    end
  end
  ...
  describe "block" do
    test "returns the list of trips on the block" do
      ...
      assert Static.block("b", "service", pid) == [
               %Trip{
                 id: "t1",
                 route_id: "route",
                 service_id: "service",
    ...
    test "returns nil if the block doesn't exist" do
      pid = Static.start_mocked(%{})
      assert Static.block("b", "service", pid) == nil
    end
  end
  ...
  describe "active_trips" do
    test "returns trips that are active right now" do
      ...
      assert [%Trip{id: "now"}] = Static.active_trips(time0 + 2, time0 + 2, pid)
    end
  end
  ...
  describe "active_blocks" do
    test "returns only blocks that are active" do
      ...
      assert %{~D[2019-01-01] => [[%Trip{id: "now", block_id: "now"}]]} =
               Static.active_blocks(time0 + 1, time0 + 3, pid)
    end
  end
  ...
  describe "shapes" do
    test "returns the shapes for all trips corresponding to the requested route" do
      ...
      assert Static.shapes("route", pid) == [
               %Shape{
                 id: "shape1",
                 points: [
                   %Point{
  ...
  describe "shape_for_trip" do
    test "returns the shape for the trip" do
      ...
      assert Static.shape_for_trip("trip", pid) ==
               %Shape{
                 id: "shape",
                 points: [
                   %Point{
  ...
  describe "first_route_pattern_for_route_and_direction" do
    test "returns the first route pattern matching the route and direction" do
      ...
      assert Static.first_route_pattern_for_route_and_direction(
  ...
  describe "fetch_remote_files" do
    test "successfully loads empty data" do
      ...
      assert {:files, _all_files} = Static.fetch_remote_files()
    end
  end
  ...
  describe "fetch_zip" do
    test "fetches and unzips zip file" do
      ...
      assert Static.fetch_zip(url, ["f"]) == {:ok, %{"f" => "x"}}
    end
  end
end
